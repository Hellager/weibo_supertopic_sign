 ...
 * @LastEditTime: 2022-06-23 14:20:03
...
async function get_follow_list(cookie) {
    ...
    data = form_params(cookie, 'cookie');
    ...
    for (let i = 0; i < topic_list.length; i++) {
        ...
        if (item.sign_status === '已签') {
            ...
            await axios({
                ...
                if (res_obj.errno != undefined) {
                    log.info(`超话 ${item.title} 签到失败 ${index} / ${list_length}, 错误提示：${res_obj.errmsg}`);
            ...
            .catch(err => {
                log.info(`超话 ${item.title} 签到失败 ${index} / ${list_length}, 错误提示：${err}`);
